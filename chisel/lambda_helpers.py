...
def unwrap_sns_event(lambda_func):
    ...
    def new_lambda_func(event, *pargs, **kwargs):
        try:
            ...
            logger.debug('Unwrapped SNS message with %d records',
                len(event['Records']))
            return result
        except KeyError as err:
            logger.debug('No SNS message to unwrap: %s', err)
    ...
    def new_lambda_func(event, *pargs, **kwargs):
        try:
            result = [lambda_func(r['s3'], *pargs, **kwargs) \
                for r in event['Records']]
            logger.debug('Unwrapped S3 message with %d records',
                len(event['Records']))
            return result
        except KeyError as err:
            logger.debug('No SNS message to unwrap: %s', err)
...
def add_logger(lambda_func):
    ...
    def new_lambda_func(event, context):
        logger = logging.getLogger(lambda_func.__name__)
        return lambda_func(event, context, logger)
    return new_lambda_func
